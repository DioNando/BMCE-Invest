...
class QuestionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Verify that the user's organization is part of this meeting
        $isPartOfMeeting = $meeting->organizations()
                                   ->where('organizations.id', $user->organization->id)
                                   ->exists();

        if (!$isPartOfMeeting) {
            return back()->with('error', 'You cannot add questions to this meeting.');
        }
